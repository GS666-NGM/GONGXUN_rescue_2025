...
class OdomSubTF : public rclcpp::Node {
public:
  OdomSubTF(std::string name) : Node(name) {
    // 创建 odom 话题订阅者，使用传感器数据的 Qos
    odom_subscribe_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "odom", rclcpp::SensorDataQoS(),
        std::bind(&OdomSubTF::odom_callback_, this, std::placeholders::_1));
...
int main(int argc, char **argv) {
  ...
  auto node = std::make_shared<OdomSubTF>("odom2tf");
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}